import { type Usuario, type TrilhaAprendizado, type Integrante, type ContatoForm } from '../types/workmind';
import { COURSES_DATA } from '../data/coursesData';
import { mockFetch } from './apiMock'; 
import imgGustavo from '../assets/gustavo.jpeg';
import imgHenrique from '../assets/henrique.jpeg';
import imgRafael from '../assets/rafael.jpeg'; 

// URL base da API real
const API_BASE_URL = 'https://workmind-java.onrender.com/api/usuarios';

export async function fetchAllUsers(): Promise<Usuario[]> {
    const response = await fetch(API_BASE_URL);
    if (!response.ok) {
        throw new Error(`Erro ao listar usuários: ${response.statusText}`);
    }
    return response.json() as Promise<Usuario[]>;
}

export async function createNewUser(userData: Omit<Usuario, 'id' | 'pontuacao'>): Promise<Usuario> {
    const response = await fetch(API_BASE_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(userData),
    });

    if (!response.ok) {
        throw new Error(`Erro ao criar usuário: ${response.statusText}`);
    }
    return response.json() as Promise<Usuario>;
}

export async function fetchUserById(id: number): Promise<Usuario> {
    const response = await fetch(`${API_BASE_URL}/${id}`);
    if (!response.ok) {
        throw new Error(`Usuário ID ${id} não encontrado: ${response.statusText}`);
    }
    return response.json() as Promise<Usuario>;
}

export async function updateScore(userId: number, pontos: number): Promise<Usuario> {
    const url = `${API_BASE_URL}/${userId}/pontuacao?pontos=${pontos}`;
    const response = await fetch(url, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pontos }), 
    });

    if (!response.ok) {
        throw new Error(`Erro ao atualizar pontuação: ${response.statusText}`);
    }
    return response.json() as Promise<Usuario>;
}

export async function deleteUser(id: number): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/${id}`, {
        method: 'DELETE',
    });

    if (!response.ok) {
        if (response.status !== 204) {
            throw new Error(`Erro ao deletar usuário: ${response.statusText}`);
        }
    }
}

export async function fetchAllCourses(): Promise<TrilhaAprendizado[]> {
  return mockFetch(COURSES_DATA);
}

export async function fetchCourseById(id: number): Promise<TrilhaAprendizado> {
  const courses = await mockFetch(COURSES_DATA);
  const course = courses.find(c => c.id === id);
  
  if (!course) {
    throw new Error('Trilha de aprendizado não encontrada (Erro 404 Mock).');
  }
  return course;
}

export async function removeCourse(_courseId: number): Promise<void> {
  await mockFetch(null); 
}

export async function fetchTeamMembers(): Promise<Integrante[]> {
    const mockTeam: Integrante[] = [
        { 
          nome: 'Henrique Gonçalves Pacheco Costa', 
          rm: '562086', 
          turma: '1TDSPK', 
          fotoUrl: imgHenrique, 
          githubLink: 'https://github.com/henriquepacheco07',
          linkedinLink: 'https://www.linkedin.com/in/henrique-pacheco-825272363?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app'
        },
        { 
          nome: 'Rafael Siqueira Martins', 
          rm: '564900', 
          turma: '1TDSPA', 
          fotoUrl: imgRafael, 
          githubLink: 'https://github.com/SiqueiraMartins', 
          linkedinLink: 'https://www.linkedin.com/in/rafael-siqueira-martins-240a3430a' 
        },
        { 
          nome: 'Gustavo Bardy Casimiro', 
          rm: 'RM566136', 
          turma: '1TDSPW', 
          fotoUrl: imgGustavo, 
          githubLink: 'https://github.com/Gustavo-Casimiro', 
          linkedinLink: 'https://linkedin.com/in/gustavo-casimiro'
        },
    ];
    return mockFetch(mockTeam); 
}

export async function submitData(url: string, data: ContatoForm): Promise<{ status: string }> {
    console.log(`Simulando POST para ${url} com dados:`, data);
    return mockFetch({ status: 'success' });
}